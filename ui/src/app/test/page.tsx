...
export default function TestGatewayPage() {
  ...
  const { connected, connecting, error, hello, request } = useGatewayConnection({
    url: urlInput,
    onHello: (helloData) => {
      ...
      <main style={{ maxWidth: "800px" }}>
        ...
        >
          ...
          <div style={{ marginTop: "1rem" }}>
            ...
            {error && (
              <p style={{ color: "var(--error)", marginTop: "0.5rem" }}>错误: {error.message}</p>
            )}
            {hello && (
              <div
                style={{
              ...
              >
                <p>
                  <strong>协议版本:</strong> {hello.protocol}
                </p>
                <p>
                  <strong>功能:</strong> {hello.features?.methods?.length || 0} 方法,{" "}
                  {hello.features?.events?.length || 0} 事件
                </p>
                {hello.auth && (
                  <p>
                    <strong>认证:</strong> {hello.auth.role} (scope: {hello.auth.scopes?.join(", ")}
                    )
                  </p>
                )}
              </div>
        ...
        >
          ...
          <ol style={{ marginLeft: "1.5rem", marginTop: "1rem", lineHeight: "1.8" }}>
            ...
            <li>点击&ldquo;连接&rdquo;按钮建立 WebSocket 连接</li>
            <li>连接成功后，可以看到协议版本和认证信息</li>
            <li>点击&ldquo;发送状态请求&rdquo;测试 API 调用</li>
          </ol>
        </section>
      </main>
