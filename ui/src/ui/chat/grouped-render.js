...
export function renderStreamingGroup(text, startedAt, onOpenSidebar, assistant) {
    ...
    <div class="chat-group assistant">
      ...
      <div class="chat-group-messages">
        ...
        timestamp: startedAt,
    }, { isStreaming: true, showReasoning: false }, onOpenSidebar)}
...
export function renderMessageGroup(group, opts) {
    ...
    <div class="chat-group ${roleClass}">
      ...
      <div class="chat-group-messages">
        ${group.messages.map((item, index) => renderGroupedMessage(item.message, {
    ...
    const initial = normalized === "user"
        ...
        : normalized === "assistant"
            ...
            : normalized === "tool"
                ? "⚙"
                : "?";
    const className = normalized === "user"
        ? "user"
        : normalized === "assistant"
            ? "assistant"
            : normalized === "tool"
...
function isAvatarUrl(value) {
    return (/^https?:\/\//i.test(value) || /^data:image\//i.test(value) || /^\//.test(value) // Relative paths from avatar endpoint
    );
}
