export async function loadChannels(state, probe) {
...
export async function startWhatsAppLogin(state, force) {
...
export async function waitWhatsAppLogin(state) {
  if (!state.client || !state.connected || state.whatsappBusy) return;
  state.whatsappBusy = true;
  try {
...
export async function logoutWhatsApp(state) {
  if (!state.client || !state.connected || state.whatsappBusy) return;
  state.whatsappBusy = true;
  try {
