...
export function setTab(host, next) {
  ...
  if (next === "logs") startLogsPolling(host);
  else stopLogsPolling(host);
  if (next === "debug") startDebugPolling(host);
  else stopDebugPolling(host);
...
export async function refreshActiveTab(host) {
  ...
  if (host.tab === "instances") await loadPresence(host);
  if (host.tab === "sessions") await loadSessions(host);
  if (host.tab === "cron") await loadCron(host);
  if (host.tab === "skills") await loadSkills(host);
  if (host.tab === "nodes") {
