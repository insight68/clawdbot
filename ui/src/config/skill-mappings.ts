...
export type SkillInteraction = {
  ...
  // 预设提示词（当 type=prompt 时）
  prompt?: string;
  // 工具调用参数（当 type=tool 时）
  toolParams?: Record<string, unknown>;
  // 外部链接（当 type=link 时）
  link?: string;
  // 模态框内容（当 type=modal 时）
  modal?: string;
};

// 单个 Skill 映射条目
export type SkillMappingEntry = {
  // 匹配规则：skillKey 支持通配符 * 和前缀匹配
  skillKeyPattern: string;
  // 显示名称（覆盖后端返回的 name）
  displayName?: string;
  // 描述（覆盖后端返回的 description）
  description?: string;
  // 分类
...
export function findSkillMapping(skillKey: string): SkillMappingEntry | undefined {
  return MARKETING_SKILL_MAPPINGS.find((entry) =>
    matchSkillPattern(skillKey, entry.skillKeyPattern),
  );
}
