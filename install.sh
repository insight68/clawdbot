...
# Yongxian Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://yongxian.xyz/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one Yongxian."
...
extract_yongxian_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_yongxian_bin_conflict() {
    ...
    if mv "$bin_path" "$backup"; then
        echo -e "${WARN}→${NC} Moved existing yongxian binary to ${INFO}${backup}${NC}"
        return 0
    fi
    return 1
}

install_yongxian_npm() {
    local spec="$1"
    local log
    log="$(mktempfile)"
    if ! SHARP_IGNORE_GLOBAL_LIBVIPS="$SHARP_IGNORE_GLOBAL_LIBVIPS" npm --loglevel "$NPM_LOGLEVEL" ${NPM_SILENT_FLAG:+$NPM_SILENT_FLAG} --no-fund --no-audit install -g "$spec" 2>&1 | tee "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_yongxian_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_yongxian_bin_conflict "$conflict"; then
                SHARP_IGNORE_GLOBAL_LIBVIPS="$SHARP_IGNORE_GLOBAL_LIBVIPS" npm --loglevel "$NPM_LOGLEVEL" ${NPM_SILENT_FLAG:+$NPM_SILENT_FLAG} --no-fund --no-audit install -g "$spec"
                return $?
            fi
            echo -e "${ERROR}npm failed because an yongxian binary already exists.${NC}"
            if [[ -n "$conflict" ]]; then
...
map_legacy_env "YONGXIAN_TAGLINE_INDEX" "CLAWDBOT_TAGLINE_INDEX"
map_legacy_env "YONGXIAN_NO_ONBOARD" "CLAWDBOT_NO_ONBOARD"
map_legacy_env "YONGXIAN_NO_PROMPT" "CLAWDBOT_NO_PROMPT"
map_legacy_env "YONGXIAN_DRY_RUN" "CLAWDBOT_DRY_RUN"
map_legacy_env "YONGXIAN_INSTALL_METHOD" "CLAWDBOT_INSTALL_METHOD"
map_legacy_env "YONGXIAN_VERSION" "CLAWDBOT_VERSION"
map_legacy_env "YONGXIAN_BETA" "CLAWDBOT_BETA"
map_legacy_env "YONGXIAN_GIT_DIR" "CLAWDBOT_GIT_DIR"
map_legacy_env "YONGXIAN_GIT_UPDATE" "CLAWDBOT_GIT_UPDATE"
map_legacy_env "YONGXIAN_NPM_LOGLEVEL" "CLAWDBOT_NPM_LOGLEVEL"
map_legacy_env "YONGXIAN_VERBOSE" "CLAWDBOT_VERBOSE"
map_legacy_env "YONGXIAN_PROFILE" "CLAWDBOT_PROFILE"
map_legacy_env "YONGXIAN_INSTALL_SH_NO_RUN" "CLAWDBOT_INSTALL_SH_NO_RUN"
...
pick_tagline() {
    ...
    if [[ -n "${YONGXIAN_TAGLINE_INDEX:-}" ]]; then
        if [[ "${YONGXIAN_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((YONGXIAN_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${YONGXIAN_NO_ONBOARD:-0}
NO_PROMPT=${YONGXIAN_NO_PROMPT:-0}
DRY_RUN=${YONGXIAN_DRY_RUN:-0}
INSTALL_METHOD=${YONGXIAN_INSTALL_METHOD:-}
YONGXIAN_VERSION=${YONGXIAN_VERSION:-latest}
USE_BETA=${YONGXIAN_BETA:-0}
GIT_DIR_DEFAULT="${HOME}/openclaw"
GIT_DIR=${YONGXIAN_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${YONGXIAN_GIT_UPDATE:-1}
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${YONGXIAN_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${YONGXIAN_VERBOSE:-0}"
YONGXIAN_BIN=""
HELP=0

print_usage() {
    cat <<EOF
Yongxian installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://yongxian.xyz/install.sh | bash -s -- [options]
...
Environment variables:
  YONGXIAN_INSTALL_METHOD=git|npm
  YONGXIAN_VERSION=latest|next|<semver>
  YONGXIAN_BETA=0|1
  YONGXIAN_GIT_DIR=...
  YONGXIAN_GIT_UPDATE=0|1
  YONGXIAN_NO_PROMPT=1
  YONGXIAN_DRY_RUN=1
  YONGXIAN_NO_ONBOARD=1
  YONGXIAN_VERBOSE=1
  YONGXIAN_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
  SHARP_IGNORE_GLOBAL_LIBVIPS=0|1    Default: 1 (avoid sharp building against global libvips)

Examples:
  curl -fsSL --proto '=https' --tlsv1.2 https://yongxian.xyz/install.sh | bash
  curl -fsSL --proto '=https' --tlsv1.2 https://yongxian.xyz/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://yongxian.xyz/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
detect_yongxian_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"yongxian"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
# Check for existing Yongxian installation
check_existing_yongxian() {
    if [[ -n "$(type -P openclaw 2>/dev/null || true)" ]]; then
        echo -e "${WARN}→${NC} Existing Yongxian installation detected"
        return 0
    fi
    return 1
}
...
warn_shell_path_missing_dir() {
    ...
    echo -e "Docs: ${INFO}https://docs.yongxian.xyz/install#nodejs--npm-path-sanity${NC}"
}
...
install_yongxian_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/insight68/yongxian.git"

    if [[ -d "$repo_dir/.git" ]]; then
        echo -e "${WARN}→${NC} Installing Yongxian from git checkout: ${INFO}${repo_dir}${NC}"
    else
        echo -e "${WARN}→${NC} Installing Yongxian from GitHub (${repo_url})..."
    fi
...
EOF
    ...
    echo -e "${SUCCESS}✓${NC} Yongxian wrapper installed to \$HOME/.local/bin/openclaw"
...
install_openclaw() {
    local package_name="yongxian"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            YONGXIAN_VERSION="$beta_version"
            echo -e "${INFO}i${NC} Beta tag detected (${beta_version}); installing beta."
            package_name="yongxian"
        else
            YONGXIAN_VERSION="latest"
            echo -e "${INFO}i${NC} No beta tag found; installing latest."
        fi
    fi

    if [[ -z "${YONGXIAN_VERSION}" ]]; then
        YONGXIAN_VERSION="latest"
    fi

    local resolved_version=""
    resolved_version="$(npm view "${package_name}@${YONGXIAN_VERSION}" version 2>/dev/null || true)"
    if [[ -n "$resolved_version" ]]; then
        echo -e "${WARN}→${NC} Installing Yongxian ${INFO}${resolved_version}${NC}..."
    else
        echo -e "${WARN}→${NC} Installing Yongxian (${INFO}${YONGXIAN_VERSION}${NC})..."
    fi
    local install_spec=""
    if [[ "${YONGXIAN_VERSION}" == "latest" ]]; then
        install_spec="${package_name}@latest"
    else
        install_spec="${package_name}@${YONGXIAN_VERSION}"
    fi

    if ! install_yongxian_npm "${install_spec}"; then
        echo -e "${WARN}→${NC} npm install failed; cleaning up and retrying..."
        cleanup_npm_yongxian_paths
        install_yongxian_npm "${install_spec}"
    fi

    if [[ "${YONGXIAN_VERSION}" == "latest" && "${package_name}" == "yongxian" ]]; then
        if ! resolve_yongxian_bin &> /dev/null; then
            echo -e "${WARN}→${NC} npm install yongxian@latest failed; retrying yongxian@next"
            cleanup_npm_yongxian_paths
            install_yongxian_npm "yongxian@next"
        fi
    fi

    ensure_yongxian_bin_link || true

    echo -e "${SUCCESS}✓${NC} Yongxian installed"
}

# Run doctor for migrations (safe, non-interactive)
run_doctor() {
    echo -e "${WARN}→${NC} Running doctor to migrate settings..."
    local claw="${YONGXIAN_BIN:-}"
    if [[ -z "$claw" ]]; then
        claw="$(resolve_yongxian_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
...
resolve_workspace_dir() {
    local profile="${YONGXIAN_PROFILE:-default}"
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.yongxian/workspace-${profile}"
    else
        echo "${HOME}/.yongxian/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local config_path="${YONGXIAN_CONFIG_PATH:-$HOME/.yongxian/yongxian.json}"
    if [[ -f "${config_path}" || -f "$HOME/.yongxian/clawdbot.json" || -f "$HOME/.yongxian/moltbot.json" || -f "$HOME/.yongxian/moldbot.json" ]]; then
        return
    fi
...
main() {
    ...
    detected_checkout="$(detect_yongxian_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            echo -e "${WARN}→${NC} Found a Yongxian checkout, but no TTY; defaulting to npm install."
            INSTALL_METHOD="npm"
        else
            local choice=""
            choice="$(prompt_choice "$(cat <<EOF
${WARN}→${NC} Detected a Yongxian source checkout in: ${INFO}${detected_checkout}${NC}
...
EOF
            ...
            case "$choice" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set YONGXIAN_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if check_existing_yongxian; then
        is_upgrade=true
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        ...
        if npm list -g openclaw &>/dev/null; then
            ...
            npm uninstall -g yongxian 2>/dev/null || true
        ...
        install_yongxian_from_git "$repo_dir"
    else
    ...
    YONGXIAN_BIN="$(resolve_yongxian_bin || true)"
    ...
    installed_version=$(resolve_yongxian_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        echo -e "${SUCCESS}${BOLD}🦞 Yongxian installed successfully (${installed_version})!${NC}"
    else
        echo -e "${SUCCESS}${BOLD}🦞 Yongxian installed successfully!${NC}"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        echo -e "Switch to global install later: ${INFO}curl -fsSL --proto '=https' --tlsv1.2 https://yongxian.xyz/install.sh | bash -s -- --install-method npm${NC}"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                YONGXIAN_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/tty && doctor_ok=1
            else
                YONGXIAN_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                echo -e "Updating plugins (${INFO}openclaw plugins update --all${NC})..."
                YONGXIAN_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
    ...
    if command -v openclaw &> /dev/null; then
        ...
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ...
                if YONGXIAN_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
    ...
    echo ""
    echo -e "FAQ: ${INFO}https://docs.yongxian.xyz/start/faq${NC}"
}

if [[ "${YONGXIAN_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_verbose
    main
fi
