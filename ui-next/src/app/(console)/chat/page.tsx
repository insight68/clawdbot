...
export default function ChatPage() {
  ...
  const { chatShowThinking: showThinking } = useUIStore();
      ...
      <div style={{
        ...
        <label style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
          ...
          <textarea
            ...
            disabled={!connected}
            placeholder={connected ? '消息内容（回车发送，Shift+回车换行）' : '连接网关后即可开始对话…'}
            style={{
              width: '100%',
              minHeight: '80px',
              padding: '0.75rem',
        ...
        <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'flex-end' }}>
          <button
            ...
            style={{
              ...
              backgroundColor: connected && !sending && draft.trim()
                ? 'var(--accent)'
                : 'var(--bg-tertiary)',
              color: connected && !sending && draft.trim()
                ? 'white'
                : 'var(--text-tertiary)',
              border: 'none',
              borderRadius: '0.375rem',
              cursor: connected && !sending && draft.trim()
                ? 'pointer'
                : 'not-allowed',
              fontSize: '0.875rem',
              fontWeight: 500,
